...
for ($i=0; $i -lt $repos.Length; $i++) {
    ...
    {
        $repo = $parsedJson.items[0].clone_url
        ...
        If ((Test-Path "$( Get-Location )/${repo_name}/.githooks/post-checkout.ps1"))
        {
            ...
            If (-not (Test-Path "$( Get-Location )/${repo_name}/.git/hooks/post-checkout.ps1"))
            {
                New-Item -ItemType SymbolicLink -Path "$( Get-Location )/${repo_name}/.git/hooks/post-checkout.ps1" -Target "$( Get-Location )/${repo_name}/.githooks/post-checkout.ps1"

                powershell "$( Get-Location )/${repo_name}/.githooks/post-checkout.ps1" "install-script"
            }
            else
            {
                Write-Output "Repository ${repo_name} already installed"
            }
        }
        else
        {
